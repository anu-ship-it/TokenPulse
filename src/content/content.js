...
(() => {
  ...
  // Caps notifications at exactly one per "user left the tab" period,
  // regardless of how many separate silence-gaps the observer below
  // detects. Without this, harmless DOM churn after the real response
  // finishes (timestamps re-rendering, citation cards loading, etc.)
  // each looked like "a new response is ready" and fired again —
  // resetting only on an actual return to the tab is the correct
  // signal boundary, not "some amount of quiet passed."
  let awaitingUserReturn = false;
  document.addEventListener("visibilitychange", () => {
    if (document.visibilityState === "visible") awaitingUserReturn = false;
  });
